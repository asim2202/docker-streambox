...
- **Everything from one repo.** Previous versions used Bookworm + Trixie apt pinning to get newer VA-API libraries, which pulled in Trixie's glibc, libstdc++, and base-files as dependencies. This created a Frankenstein container where rebuilds pulled different package versions from rolling Trixie, causing non-reproducible builds and runtime breakage.
- Trixie (Debian 13) is the current stable release. It ships FFmpeg 7.x, libva 2.22+, and intel-media-va-driver natively -- no hacks needed.

### Chromium instead of Firefox
- **Debian's Firefox ESR has no VA-API support** -- it's compiled without it. `strings` search on libxul.so confirmed zero vaapi references.
...
### CQP rate control (`-qp 23`) instead of CBR/VBR
...
**Fix:** Restart your RTMP server to clear stale sessions. Verify the RTMP server can handle the incoming h264 stream.
...
**Expected:** DRM video decoding is done in software (see above). Total Chromium CPU for 1080p DRM video is ~60-130%.
...
**Cause:** FFmpeg crashed or was killed.
...
The GitHub Actions workflow automatically builds and pushes to `ghcr.io/asim2202/docker-streambox:latest` on push to main.
...
- **Platform:** Unraid 6.x
- **Proven stable:** 8+ hours continuous 1080p60 stream, 0 duplicate frames
